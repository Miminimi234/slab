...
const GMGN_SEARCH_DEFAULT_PARAMS = {
    ...
    client_id: "gmgn_web_20251029-6163-7261a63",
    from_app: "gmgn",
    app_ver: "20251029-6163-7261a63",
...
const PUPPETEER_PROFILE_DIR = path.resolve(process.cwd(), ".local", "puppeteer_profile");
...
async function searchGmgnCoins(query: string, chain = "sol", attempt = 0): Promise<SearchResponse> {
    ...
    const url = `${GMGN_SEARCH_ENDPOINT}?${params}`;
...
async function fetchTokenHolders(
    ...
    const params = new URLSearchParams({
        ...
        orderby: String(opts.orderby ?? "amount_percentage"),
        direction: String(opts.direction ?? "desc"),
        cost: String(typeof opts.cost === "number" ? opts.cost : 20),
    }).toString();
